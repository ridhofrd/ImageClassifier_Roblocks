...
def train():
    ...
    try:
        ...
        data_config = {
            'train': os.path.abspath(train_base_dir),  # Absolute path
            'val': os.path.abspath(val_base_dir),      # Absolute path
            'nc': len(class_names),
            'names': class_names
        }
        ...
        results = model.train(
            ...
            batch=batch_size,
            lr0=learning_rate,
            project=session_folder,   # Tell YOLO to save its run outputs here
            name='train_run',         # Subdirectory name for this specific run
            exist_ok=True,
            # cache=False # Try disabling cache just in case: cache='ram' or cache=False
        )
