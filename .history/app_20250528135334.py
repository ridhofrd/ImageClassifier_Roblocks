...
def train():
    ...
    if gpus:
        try:
            ...
            # tf.config.set_visible_devices(gpus[0], 'GPU') # Optional: if you want to be specific
            # Allow memory growth to avoid allocating all GPU memory at once
            for gpu in gpus:
                tf.config.experimental.set_memory_growth(gpu, True)
            logical_gpus = tf.config.list_logical_devices('GPU')
            app.logger.info(f"{len(gpus)} Physical GPUs, {len(logical_gpus)} Logical GPUs available.")
        except RuntimeError as e:
            # Memory growth must be set before GPUs have been initialized
            app.logger.error(f"Error during GPU setup: {e}")
    else:
        app.logger.info("No GPU found by TensorFlow. Training will use CPU.")
    # --- End GPU check ---
    ...
    try:
        ...
        for idx, file in enumerate(files):
            ...
            try: # Try to derive label from filename first
                ...
                if len(class_labels_from_form) == len(files): # This is a bit risky if order isn't guaranteed
                    ...
                    if len(class_labels_from_form) == 1: # Fallback to using the first class label if only one is provided for all images
        ...
        for class_name_item in class_labels_from_form: 
        ...
        if can_stratify:
            ...
            train_files, val_files = train_test_split(
                image_paths_original_location, test_size=0.2, random_state=42, shuffle=True # Removed image_labels_original from args here
            )
        
        for file_path in train_files:
            # Find the label for this file_path
