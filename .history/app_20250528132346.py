...
def train():
    ...
    try:
        ...
        with open(data_yaml_path, 'w') as f:
            ...
            f.flush()  # Ensure Python's internal buffers are written to the OS
            os.fsync(f.fileno()) # Tell OS to physically write to disk
        app.logger.info(f"data.yaml created and flushed at: {data_yaml_path}")


        with open(data_yaml_path, 'r') as f_read: # Log contents after flush
            app.logger.info(f"data.yaml contents (after flush):\n{f_read.read()}")
