...
def train():
    ...
    g.session_folder = os.path.abspath(os.path.join(UPLOAD_FOLDER, g.session_id))
    os.makedirs(g.session_folder, exist_ok=True)
    app.logger.info(f"Session folder created: {g.session_folder}")
    ...
    try:
        ...
        train_base_dir = os.path.join(g.session_folder, 'train')
        val_base_dir = os.path.join(g.session_folder, 'val')
        ...
        temp_image_save_dir = os.path.join(g.session_folder, "all_images_temp")
        ...
        keras_model_dir = os.path.join(g.session_folder, 'keras_saved_model')
        ...
        temp_tflite_path = os.path.join(g.session_folder, 'model.tflite')
        ...
        #     (g.session_id, class_labels, model_path, created_at)
        #     VALUES (%s, %s, %s, %s)
        # """
        # execute_query(query, (g.session_id, ','.join(actual_class_names), db_model_path, int(datetime.now().timestamp() * 1000)), commit=True)
