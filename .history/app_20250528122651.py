...
def train():
    ...
    try:
        # --- Define class_names and other parameters first ---
        ...
        class_names = sorted(list(set(class_labels_form))) # Define class_names here
        app.logger.info(f"Class names defined: {class_names}")
        # --- End of initial setup of class_names and parameters ---
        ...
        # Logic for processing images and assigning labels
        # This part needs class_names to validate against if deriving from filename
        for idx, file in enumerate(files):
            ...
            try:
                # Attempt to derive class label from filename (e.g., "classA_img1.jpg")
                class_label_for_file = filename.split('_img_')[0]
                if class_label_for_file not in class_names: # Validate against defined class_names
                ...
                # Fallback if filename doesn't match pattern: use class_labels_form by order
                if len(class_labels_form) == len(files):
                    class_label_for_file = class_labels_form[idx]
                    # Ensure this assigned label is actually one of the valid class_names
            ...
            if not class_label_for_file: # Should be set by now
        ...
        # Check if all declared classes actually have images associated with them
        ...
        # Create class subdirectories using the now defined class_names
        for class_name_item in class_names: # class_names is used here
        ...
        data_config = {
            ...
            'nc': len(class_names),    # class_names is used here
            'names': class_names       # class_names is used here
        }
