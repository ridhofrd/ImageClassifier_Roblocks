...
def train():
    ...
    try:
        ...
        with open(data_yaml_path, 'r') as f_read: # Log contents of data.yaml for debugging
            app.logger.info(f"data.yaml contents:\n{f_read.read()}")


        os.chdir(session_folder) # Change CWD to session_folder
        app.logger.info(f"Changed CWD to: {os.getcwd()}")

        model = YOLO('yolov8n-cls.pt')
        
        model_output_project_path = '.' # Save 'runs' in the current CWD (session_folder)

        results = model.train(
            data='data.yaml',  # Path is relative to CWD (session_folder/data.yaml)
            epochs=epochs,
            imgsz=640,
            batch=batch_size,
            lr0=learning_rate,
            project=model_output_project_path, 
            name='train_run',
            exist_ok=True
        )
        
        trained_model_path_pt = os.path.join(model_output_project_path, 'train_run', 'weights', 'best.pt')
        # Since CWD is session_folder, and project is '.', path becomes:
