...
def train():
    ...
    try:
        ...
        _INPUT_NORM_MEAN = 0.0  # Matches Rescaling(1./255) -> [0,1]
        _INPUT_NORM_STD = 1.0
        _LABEL_FILE = os.path.join(session_folder, 'labels.txt')
        ...
        writer = ImageClassifierWriter.create_for_inference(
            writer_utils.load_file(temp_tflite_path),
            np.array([_INPUT_NORM_MEAN] * 3, dtype=np.float32),
            np.array([_INPUT_NORM_STD] * 3, dtype=np.float32),
            [_LABEL_FILE]
        )
        writer_utils.save_file(writer.populate(), temp_tflite_path)
        app.logger.info(f"TFLite model with metadata saved to: {temp_tflite_path}")
