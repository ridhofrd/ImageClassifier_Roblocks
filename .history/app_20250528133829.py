...
def train():
    ...
    try:
        ...
        class_labels_from_form = sorted(list(set(request.form.getlist('class_label'))))
        epochs = int(request.form.get('epochs', 10)) # Default to 10 epochs
        batch_size = int(request.form.get('batch_size', 32)) # Default to 32
        learning_rate = float(request.form.get('learning_rate', 0.001)) # Default LR
        ...
        image_labels_original = [] # Labels corresponding to each image path
        ...
        for idx, file in enumerate(files):
            ...
            try: # Try to derive label from filename first
            ...
            except IndexError: # Fallback to form labels by order
                if len(class_labels_from_form) == len(files): # This is a bit risky if order isn't guaranteed
                    class_label_for_file = class_labels_from_form[idx] # Assuming one label per image file if names don't encode class
                    # No need to check if class_label_for_file is in class_labels_from_form, as it's taken from it.
                else: # Fallback to using the first class label if only one is provided for all images
        ...
        # --- Create train/val directory structure for TensorFlow ---
        ...
        for class_name_item in class_labels_from_form: # Use form labels to create dirs
        ...
        # Stratify based on the image_labels_original
        min_samples_per_class = min([image_labels_original.count(cls) for cls in set(image_labels_original)])
        ...
        for file_path in train_files:
            label = next(lbl for pth, lbl in zip(image_paths_original_location, image_labels_original) if pth == file_path)
        ...
        # Clean up temp_image_save_dir as files are moved
        ...
        # Get actual class names inferred by TensorFlow (important for consistency)
        ...
        # Note: EfficientNetB0 has its own preprocess_input, but Rescaling(1./255) is a common alternative.
        # For simplicity, we use Rescaling here. If using EfficientNet's specific preprocessing:
        ...
        # --- Build the model (Transfer Learning with EfficientNetB0) ---
        ...
        # x = data_augmentation(inputs) # Augmentation is now part of the dataset pipeline
        x = inputs # Data from dataset is already augmented and normalized
        # x = tf.keras.applications.efficientnet.preprocess_input(x) # If using specific preprocessing
        x = base_model(x, training=False) # training=False for base_model when frozen
        ...
        history = model.fit(
            ...
            # callbacks=[tf.keras.callbacks.EarlyStopping(patience=3, restore_best_weights=True)] # Optional
        )
        app.logger.info("TensorFlow model training completed.")

        # --- Save the Keras SavedModel ---
        keras_model_dir = os.path.join(session_folder, 'keras_saved_model.keras')
        model.save(keras_model_dir)
        app.logger.info(f"Keras SavedModel saved to: {keras_model_dir}")

        # --- Convert to TensorFlow Lite ---
        converter = tf.lite.TFLiteConverter.from_saved_model(keras_model_dir)
        converter.optimizations = [tf.lite.Optimize.DEFAULT] # Optional: optimize for size/latency
        tflite_model_content = converter.convert()

        # Save the TFLite model temporarily in session_folder
        ...
        # Move TFLite model to the final MODEL_PATH
        os.makedirs(os.path.dirname(MODEL_PATH), exist_ok=True) # Ensure target directory exists
        os.rename(temp_tflite_path, MODEL_PATH)
        app.logger.info(f"TFLite model moved to final path: {MODEL_PATH}")
        db_model_path = MODEL_PATH # Path to store in DB
        ...
        query = """
            ...
            VALUES (%s, %s, %s, %s)
        """
        # Use actual_class_names inferred by TensorFlow for DB consistency
