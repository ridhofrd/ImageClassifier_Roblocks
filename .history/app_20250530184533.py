...
def train():
    ...
    try:
        ...
        app.logger.info(f"Raw class label from form: {class_label_raw}")
        ...
        if class_label_raw:
            try:
                ...
                if isinstance(parsed, list):
                    ...
                    app.logger.error(f"Invalid class label format: {parsed}")
                    return jsonify({"error": "Invalid class label format"}), 400
            except json.JSONDecodeError:
                class_labels_from_form = [class_label_raw]
                app.logger.info(f"Using class label as string: {class_label_raw}")
        ...
        try:
            class_labels_from_form = json.loads(class_label_raw) if class_label_raw else []
            class_labels_from_form = sorted(list(set(class_labels_from_form)))
        except json.JSONDecodeError:
            app.logger.error(f"Invalid JSON format for class labels: {class_label_raw}")
            return jsonify({"error": "Invalid JSON format for class labels"}), 400
