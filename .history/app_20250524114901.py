...
# Helper function to generate YOLOv8 annotations
def generate_annotations(image_paths, labels):
...
def train():
    try:
        ...
        images_per_class = len(files) // len(class_labels)
        remainder = len(files) % len(class_labels)
        image_index = 0
        for label in class_labels:
            label_folder = os.path.join(session_folder, label)
            os.makedirs(label_folder, exist_ok=True)
            num_images = images_per_class + (1 if remainder > 0 else 0)
            for _ in range(num_images):
                if image_index >= len(files):
                    break
                file = files[image_index]
                file_path = os.path.join(label_folder, secure_filename(file.filename))
                file.save(file_path)
        ...
        if os.path.exists('best.tflite'):
            os.rename('best.tflite', MODEL_PATH)
        ...
        query = """
            ...
            VALUES (%s, %s, %s, %s)
        """
        execute_query(query, (session_id, ','.join(class_labels), MODEL_PATH, int(datetime.now().timestamp() * 1000)), commit=True)

        # Return metrics for Android app's graph
        metrics = {
            "epochs": list(range(1, epochs + 1)),
            "mAP50": results.results_dict.get('metrics/mAP50', [0.0] * epochs)
        }
        return {"status": "success", "session_id": session_id, "metrics": metrics}, 200
