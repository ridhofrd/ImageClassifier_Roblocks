...
from ultralytics import YOLO, settings as ultralytics_settings # Import settings

try:
    if hasattr(ultralytics_settings, 'update'): # For newer versions where settings is an object
        ultralytics_settings.update({'datasets_dir': ''})
        logging.info("Globally overriding ultralytics_settings.datasets_dir to an empty string.")
    elif isinstance(ultralytics_settings, dict): # For some versions, settings might be a dict
        ultralytics_settings['datasets_dir'] = ''
        logging.info("Globally setting ultralytics_settings['datasets_dir'] to an empty string (dict-like).")
    else: # Fallback: try to set an attribute if it exists
        if hasattr(ultralytics_settings, 'datasets_dir'):
            setattr(ultralytics_settings, 'datasets_dir', '')
            logging.info("Globally setting ultralytics_settings.datasets_dir attribute to an empty string.")
        else:
            logging.warning("Could not determine how to set datasets_dir on ultralytics_settings object.")
            
    # You could also try setting to None if empty string doesn't work
    # ultralytics_settings.update({'datasets_dir': None})

except Exception as e:
    logging.warning(f"Could not programmatically update Ultralytics settings at a global level: {e}")
# --- End Ultralytics settings override ---
...
def train():
    ...
    try:
        ...
        train_base_dir = os.path.join(session_folder, 'train') # This path is absolute
        val_base_dir = os.path.join(session_folder, 'val')   # This path is absolute
        ...
        for img_path, label in zip(val_images_paths, val_labels_list): # Error in your previous code, was missing os.rename
            dest_path = os.path.join(val_base_dir, label, os.path.basename(img_path))
            os.rename(img_path, dest_path) # Added os.rename here
        ...
        # --- Create data.yaml with RELATIVE paths for os.chdir() strategy ---
        data_config = {
            'train': 'train',  # Relative to session_folder (which will be CWD)
            'val': 'val',      # Relative to session_folder (which will be CWD)
            'nc': len(class_names),
            'names': class_names
        }
        ...
        # --- End data.yaml creation ---

        os.chdir(session_folder) # Change CWD for YOLO training
        ...
        # trained_model_path_pt_abs needs to be constructed carefully based on CWD and project='.'
        # Since CWD is session_folder, and project is '.', path is session_folder/train_run/weights/best.pt
        trained_model_path_pt_abs = os.path.join(session_folder, 'train_run', 'weights', 'best.pt')
        
        # No need to change CWD back yet, do it in finally block
