...
def train():
    ...
    if os.path.exists(session_folder):
        app.logger.warning(f"Session folder already exists: {session_folder}. This will overwrite existing data.")
        shutil.rmtree(session_folder)  
        app.logger.info(f"Existing session folder removed: {session_folder}")  
    ...
    try:
        ...
        final_model_dir = temp_tflite_path
        os.makedirs(final_model_dir, exist_ok=True)
        if os.path.exists(MODEL_PATH):
            os.remove(MODEL_PATH)
            app.logger.info(f"Old model removed from: {MODEL_PATH}")
        
        # Move the new model to the final path
        os.rename(temp_tflite_path, MODEL_PATH)
        app.logger.info(f"Final TFLite model moved to: {MODEL_PATH}")

        # Optional: Log session folder to track saved training state
        app.logger.info(f"Training session saved at: {session_folder}")
        ...
        app.logger.exception("Unexpected error during training process")
